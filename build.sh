...
if [ "$1" = "dev" ]; then
  ...
  webVersion=$(wget -qO- -t1 -T2 "https://api.github.com/repos/li-peifeng/iSweet-Web/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
fi
...
FetchWebDev() {
  curl -L https://codeload.github.com/li-peifeng/iSweet-Dist/tar.gz/refs/heads/dev -o web-dist-dev.tar.gz
