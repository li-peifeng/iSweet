...
func FsList(c *gin.Context) {
	...
	if !common.CanAccess(user, meta, reqPath, req.Password) {
		common.ErrorStrResp(c, "密码不正确或没有权限" + "Password is incorrect or you have no permission", 403)
		return
	}
	if !user.CanWrite() && !common.CanWrite(meta, reqPath) && req.Refresh {
		common.ErrorStrResp(c, "没有权限刷新" + "Refresh without permission", 403)
		return
	}
...
func FsDirs(c *gin.Context) {
	...
	if req.ForceRoot {
		if !user.IsAdmin() {
			common.ErrorStrResp(c, "访问被拒绝" + "Permission denied", 403)
			return
		}
	} else {
